            ...
            if(strlen($username)>=5){
                if(strlen($password)>=8 && strlen($c_password)>=8){
                        if($password==$c_password){
                                $user_check=mysqli_query($admin_dbcon,"SELECT * FROM `student_table` WHERE `username`");
                                if(mysqli_num_rows($user_check)==0){
                                    $email_check=mysqli_query($admin_dbcon,"SELECT * FROM `student_table` WHERE `email`='$email'");     
                                        if(mysqli_num_rows($email_check)==0){
												...
												$insert_query=mysqli_query($admin_dbcon,"INSERT INTO `student_table`(`username`, `email`, `mobile`, `password`, `register time`, `status`) VALUES ('$username','$email','$mobile','$password','$register time','Inactive')");
												if($insert_query){
													echo '<script>
...
<body>
	...
	<div class="left">
		...
		<div class="right"> 
			<img src="image/Login-rafiki.png" alt="" />
		</div>
</div>
    </div>
